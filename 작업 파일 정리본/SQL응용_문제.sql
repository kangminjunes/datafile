...
SELECT PROF_NAME AS 교수명, 
       TRUNC(TO_DATE('2020-12-31')- PROF_HIRED) AS 근속일
  FROM PROFESSOR_T;
...
--500       정보미디어학과     0
--800       응용소프트웨어학과 0
SELECT D.DEPT_NO   AS 학과번호 ,
       D.DEPT_NAME AS 학과명, 
       COUNT(S.STUD_NO) AS 학생수
FROM DEPARTMENT_T D
LEFT JOIN STUDENT_T S ON D.DEPT_NO = S.DEPT_NO
...
--조명훈  전자공학과
--류민정  컴퓨터공학과
SELECT S.STUD_NAME AS 학생명, 
       D.DEPT_NAME AS 학과명
FROM STUDENT_T S
INNER JOIN DEPARTMENT_T D ON S.DEPT_NO = D.DEPT_NO;
...
--6     이재우  4200
--6     이만식  4200
SELECT ROW_NUMBER() OVER (ORDER BY PROF_SAL DESC) AS 순위,
       PROF_NAME AS 교수명,
       PROF_SAL  AS 연봉
  FROM PROFESSOR_T;
